build_image() {
    ...
    docker build --build-arg env=${env} -t ${tag} -f ./deploy/worker/Dockerfile . && \
    docker push ${tag}
}
...
case "${ENV}" in
    'dev')
        ...
        build_image dev private-registry.sohucs.com/kuaizhan-plf/weixin-worker:dev
    ;;
    'test')
        # build_image test registry.kuaizhan.com/kuaizhan-plf/weixin-worker:test
        build_image test private-registry.sohucs.com/kuaizhan-plf/weixin-worker:test
    ;;
    'pre')
